...
def extract_fields(text: str) -> Dict[str, Optional[str]]:
    """Extract common packaged-commodity declarations from raw OCR text."""
    t = text.replace("\r", "")
    # Normalize common OCR confusions without rewriting arbitrary product text.
    t = t.replace("\u00a0", " ").replace("Rs ", "Rs. ")
    t = re.sub(r"\bN\s*E\s*T\s*Q\s*T\s*Y\b", "Net Quantity", t, flags=re.I)
    t = re.sub(r"\bN\s*E\s*T\s*(?:Q\s*T\s*Y|Q\s*T)\b", "Net Quantity", t, flags=re.I)
    t = re.sub(r"\bM\s*\.?\s*R\s*\.?\s*P\b", "MRP", t, flags=re.I)
    t = re.sub(r"\bC\s*O\s*U\s*N\s*T\s*R\s*Y\s*OF\s*O\s*R\s*I\s*G\s*I\s*N\b", "Country of Origin", t, flags=re.I)
    t = re.sub(r"\bF\s*S\s*S\s*A\s*I\b", "FSSAI", t, flags=re.I)
    out = {k: None for k in FIELDS}
    lines = [re.sub(r"[ \t]+", " ", ln).strip(" -\t") for ln in t.split("\n") if ln.strip()]
    joined = "\n".join(lines)

    def first(pattern: str, flags=re.I) -> Optional[str]:
        m = re.search(pattern, joined, flags)
        return m.group(1).strip(" \t:;-.") if m else None

    out["product_name"] = first(r"(?:^|\n)\s*(?:product(?:\s*/?\s*common\s*name)?|product\s*name|common\s*name)\s*[:\-]?\s*([^\n]+)")
    out["manufacturer"] = first(r"(?:manufacturer|manufactured\s*by)\s*[:\-]?\s*([^\n]+)")
    out["packer"] = first(r"(?:packer|packed\s*by)\s*[:\-]?\s*([^\n]+)")
    out["importer"] = first(r"(?:importer|imported\s*by)\s*[:\-]?\s*([^\n]+)")

    m_addr = re.search(r"address\s*[:\-]?\s*(.+?)(?=\n(?:net\s*(?:qty|quantity|wt)|mrp|max(?:imum)?\s*retail|packed|manufactured|best\s*before|use\s*by|batch|consumer\s*care|customer\s*care|country\s*of\s*origin|made\s*in|unit\s*(?:sale\s*)?price)\b|$)", joined, re.I | re.S)
    if m_addr:
        out["address"] = re.sub(r"\s+", " ", m_addr.group(1)).strip(" :;,-")

    out["net_quantity"] = first(r"(?:net\s*(?:qty|quantity|wt)|net\s*weight)\s*[:\-]?\s*([0-9]+(?:\.[0-9]+)?\s*(?:kg|g|mg|ml|l|litre|liter|pcs|pc))")
    mrp_match = re.search(r"(?:m\.?\s*r\.?\s*p\.?|maximum\s*retail\s*price)\s*[:\-]?\s*(?:₹|rs\.?|inr|[^0-9\s])?\s*([0-9]+(?:\.[0-9]+)?)", joined, re.I)
    if mrp_match:
        raw_mrp = mrp_match.group(1).strip()
        # OCR often reads the ₹ glyph as a leading '2'/'?' before the true amount.
        if len(raw_mrp) >= 3 and raw_mrp.startswith("2") and not re.search(r"(?:\.\d+)$", raw_mrp):
            candidate = raw_mrp[1:]
            if candidate and float(candidate) < 100000:
                raw_mrp = candidate
        out["mrp"] = raw_mrp
    out["packed_date"] = first(r"(?:packed|packed\s*on|date\s*of\s*(?:packing|manufacture)|manufactured\s*on|mfd\.?|pkd\.?)\s*[:\-]?\s*([0-9]{1,2}[/-][0-9]{1,2}[/-][0-9]{2,4}|[0-9]{1,2}/[0-9]{4}|[A-Za-z]{3,9}\s+[0-9]{4})")
    out["best_before"] = first(r"(?:best\s*before|use\s*by)\s*[:\-]?\s*([^\n]+)")
    out["batch_number"] = first(r"(?:batch|lot)\s*(?:no\.?|number)?\s*[:\-]?\s*([A-Za-z0-9][A-Za-z0-9\-/]{2,30})")
    out["country_of_origin"] = first(r"(?:country\s*of\s*origin|made\s*in)\s*[:\-]?\s*([A-Za-z][A-Za-z .'-]{1,50})")
    usp_match = re.search(r"(?:unit\s*sale\s*price|unit\s*price|usp)\s*[:\-=%₹rs\.inr]*\s*([0-9]+(?:\.[0-9]+)?)\s*(?:(?:/|per)\s*[A-Za-z]+)?", joined, re.I)
    if usp_match:
        raw_usp = usp_match.group(1).strip()
        out["unit_sale_price"] = raw_usp
    else:
        usp_hint = re.search(r"\busp\b.*?([0-9]+(?:\.[0-9]+)?)\s*(?:per|/|\b)", joined, re.I)
        if usp_hint:
            out["unit_sale_price"] = usp_hint.group(1)

    phone_matches = re.findall(r"(?<!\d)(?:\+?91[\s-]?)?[6-9]\d{9}(?!\d)|(?<!\d)1800[\s-]?\d{3,4}[\s-]?\d{3,5}(?!\d)", joined)
    if phone_matches:
        out["consumer_phone"] = re.sub(r"\s+", " ", phone_matches[-1]).strip()
    email_matches = re.findall(r"[A-Z0-9._%+\-]+@[A-Z0-9.\-]+\.[A-Z]{2,}", joined, re.I)
    if email_matches:
        out["consumer_email"] = email_matches[-1]

    consumer = first(r"(?:consumer\s*care|customer\s*care|helpline|contact)\s*[:\-]?\s*([^\n]+)")
    if consumer:
        parts = [consumer]
    ...
    elif out["consumer_phone"] or out["consumer_email"]:
        out["consumer_care"] = " | ".join([x for x in (out["consumer_phone"], out["consumer_email"]) if x])

    # Preserve other visible declarations without pretending they are mandatory fields.
    extras = []
    for ln in lines:
        low = ln.lower()
        if any(k in low for k in ("ingredients", "allergen", "license", "fssai", "barcode", "veg", "non-veg", "storage", "warning")):
            extras.append(ln)
    if extras:
        out["other_declarations"] = " | ".join(extras[:8])

    # Recover values when OCR drops punctuation or the label keyword.
    if not out["product_name"]:
        candidates = []
        for ln in lines:
            low = ln.lower()
            if any(k in low for k in ("packaged commodity", "nutrition", "fssai", "mrp", "net qty", "manufacturer", "consumer care", "passport", "digital product", "registry", "verification", "status", "declarations", "history", "overview", "evidence", "lic. no", "license no")):
                continue
            if 2 <= len(ln.split()) <= 8 and not re.fullmatch(r"[0-9 .%:/\-]+", ln):
                candidates.append(ln)
        if candidates:
            product_terms = ("rice", "biscuit", "chips", "shampoo", "flavour", "food", "namkeen", "juice", "oil", "soap", "tea", "coffee", "masala")
            brand_terms = ("lays", "lay's", "sunrise", "abc")
            weighted = sorted(candidates, key=lambda x: (
                30 * sum(k in x.lower() for k in brand_terms) +
                20 * sum(k in x.lower() for k in product_terms) +
                min(len(x), 60)
            ), reverse=True)
            out["product_name"] = weighted[0]
    if not out["address"]:
        addr_bits = []
        for ln in lines:
            low = ln.lower()
            if any(k in low for k in ("plot no", "sector", "industrial estate", "road", "nagar", "pincode", "pradesh", "mumbai", "chennai", "pune", "parwanoo")) and not any(k in low for k in ("product", "nutrition")):
                addr_bits.append(ln)
        if addr_bits:
            unique_addr = []
            for bit in addr_bits:
                if not any(SequenceMatcher(None, re.sub(r"\W", "", bit.lower()), re.sub(r"\W", "", prev.lower())).ratio() > 0.9 for prev in unique_addr):
                    unique_addr.append(bit)
            out["address"] = " ".join(unique_addr[:3])
    if not out["batch_number"]:
        m = re.search(r"\b(?:mfg|batch|lot)[ ._-]*([A-Z0-9]{4,12})\b", joined, re.I)
        if m: out["batch_number"] = m.group(1)
    if not out["consumer_phone"]:
        m = re.search(r"\b1800[\s:.-]*[0-9]{2,4}[\s:.-]*[0-9]{3,5}\b", joined)
        if m: out["consumer_phone"] = re.sub(r"\D", "", m.group(0))
    if out["consumer_phone"] and not out["consumer_care"]:
...
def _ocr_variants(img: Image.Image) -> List[tuple[str, int, str]]:
    ...
    for name, im in variants:
        for psm in (6, 11):
            try:
                ...
                continue
    # Focused crops: one high-recall sparse-text pass each.
    for name, im in crops:
        try:
            ...
            text = "\n".join(" ".join(words) for words in rows.values()).strip()
            mean = round(sum(confs) / len(confs)) if confs else 0
            if text: passes.append((text, mean, f"crop-{name}"))
        except Exception:
            continue
    return passes
...
def ocr_image(path: Path) -> tuple[str, Dict[str, int], Dict[str, Dict[str, int]]]:
    """High-recall OCR with PaddleOCR as primary, plus Tesseract consensus/fallback."""
    try:
        img = Image.open(path).convert("RGB")
        candidates: List[tuple[str, int, str]] = []

        # Primary OCR: PaddleOCR 3.x text detection + recognition.
        paddle = _paddle_ocr_text(path)
        if paddle:
            candidates.append((paddle[0], paddle[1], "paddle-primary"))

        # Secondary OCR: multiple Tesseract layouts/preprocessing passes.
        if PYTESS_AVAILABLE:
            candidates.extend(_ocr_variants(img))

        candidates = [(t, c, n) for t, c, n in candidates if t.strip()]
        # Give PaddleOCR a small preference because it is the primary engine, while preserving
        # Tesseract text that can recover small/low-contrast legal fields.
        candidates.sort(key=lambda x: ((1 if x[2].startswith("paddle") else 0), min(len(x[0]), 900) * 0.55 + x[1] * 7.0), reverse=True)
        chosen = candidates[:10]
        text = _dedupe_texts([c[0] for c in chosen])
...
@app.get("/api/health")
def health():
    return {"status":"ok", "ocr":"PaddleOCR primary + multi-pass Tesseract cross-check", "ocr_available":PYTESS_AVAILABLE, "paddle_available":PADDLE_AVAILABLE, "paddle_initialized":_get_paddle_engine() is not None if PADDLE_AVAILABLE else False, "paddle_init_error":_PADDLE_INIT_ERROR, "opencv_available":CV2_AVAILABLE, "product":"PackCheck AI", "version":"2.0.0"}
...
async def upload(file: UploadFile = File(...)):
    ...
    return {
        ...
        "readability_status":quality_status,
        "readability_score":quality_score,
        "ocr_available": PYTESS_AVAILABLE and bool(ocr_text.strip()),
        "ocr_text": ocr_text,
        "ocr_mean_confidence": ocr_meta.get("__mean__", 0),
        "boxes": boxes,
        "message":"Upload accepted. OCR runs locally when the Tesseract desktop engine is available."
    }
...
def create_passport(scan_id: str):
    ...
    if existing:
        c.close(); return passport_payload(dict(existing))
    ...
    gtin=scan["fields"].get("other_declarations") or "Not detected"
    payload=json.dumps({"passport_id":pid,"scan_id":scan_id,"product_name":product,"gtin":gtin,"rule_version":scan.get("rule_version"),"created_at":now()},sort_keys=True)
    ...
    return passport_payload(dict(row))


def passport_payload(row: dict):
    payload=json.loads(row["signed_payload"]); valid=hmac.compare_digest(passport_signature(row["signed_payload"]),row["signature"])
    return {**row,"payload":payload,"signature_valid":valid,"registry_url":f"/api/passports/{row['passport_id']}","qr_url":f"/api/passports/{row['passport_id']}/qr","public_url":f"/passport/{row['passport_id']}"}
...
def passport_public_url(passport_id: str, request: Request) -> str:
